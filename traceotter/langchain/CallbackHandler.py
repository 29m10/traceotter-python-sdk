...
from traceotter.langchain.utils import _extract_model_name
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the Traceotter langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(
        ...
        Args:
            public_key: Optional Traceotter public key. If not provided, will use the default client configuration.
            update_trace: Whether to update the Traceotter trace with the chains input / output / metadata / name. Defaults to False.
        """
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            run_id in self.runs
            and isinstance(self.runs[run_id], TraceotterGeneration)
            and run_id not in self.updated_completion_start_time_memo
        ):
            current_generation = cast(TraceotterGeneration, self.runs[run_id])
    ...
    def _get_observation_type_from_serialized(
        ...
        """Determine Traceotter observation type from LangChain component.
        ...
        Returns:
            The appropriate Traceotter observation type string
        """
    ...
    def on_retriever_error(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            traceotter_logger.exception(e)

    def _parse_traceotter_trace_attributes_from_metadata(
        self,
        ...
        if "traceotter_session_id" in metadata and isinstance(
            metadata["traceotter_session_id"], str
        ):
            attributes["session_id"] = metadata["traceotter_session_id"]

        if "traceotter_user_id" in metadata and isinstance(
            metadata["traceotter_user_id"], str
        ):
            attributes["user_id"] = metadata["traceotter_user_id"]

        if "traceotter_tags" in metadata and isinstance(metadata["traceotter_tags"], list):
            attributes["tags"] = [str(tag) for tag in metadata["traceotter_tags"]]
        ...
        try:
            ...
            if parent_run_id is None:
                span.update_trace(
                    ...
                    **self._parse_traceotter_trace_attributes_from_metadata(metadata),
                )
    ...
    def _register_traceotter_prompt(
        ...
        *,
        ...
        """We need to register any passed Traceotter prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        traceotter_prompt = metadata and metadata.get("traceotter_prompt", None)

        if traceotter_prompt:
            self.prompt_to_parent_run_map[parent_run_id] = traceotter_prompt
    ...
    def _deregister_traceotter_prompt(self, run_id: Optional[UUID]) -> None:
        ...
        try:
            ...
            if agent_run is not None:
                agent_run._otel_span.set_attribute(
                    TraceotterOtelSpanAttributes.OBSERVATION_TYPE, "agent"
                )
        ...
        try:
            ...
            content = {
                ...
                "metadata": self.__join_tags_and_metadata(
                    ...
                    keep_traceotter_trace_attributes=True
                    if parent_run_id is None
                    else False,
                ),
                "model": model_name,
    ...
    def _log_model_parse_warning(self) -> None:
        if not hasattr(self, "_model_parse_warning_logged"):
            traceotter_logger.warning(
                "Traceotter was not able to parse the LLM model. The LLM call will be recorded without model name. Please create an issue: https://github.com/traceotter/traceotter/issues/new/choose"
            )
    ...
    def __join_tags_and_metadata(
        ...
        return (
            _strip_traceotter_keys_from_dict(final_dict, keep_traceotter_trace_attributes)
            if final_dict != {}
            else None
        )
...
def _parse_usage_model(usage: typing.Union[pydantic.BaseModel, dict]) -> Any:
    ...
    for model_key, traceotter_key in conversion_list:
        if model_key in usage_model:
            ...
            usage_model[traceotter_key] = final_count  # Translate key and keep the value
...
def _strip_traceotter_keys_from_dict(
    metadata: Optional[Dict[str, Any]], keep_traceotter_trace_attributes: bool
) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata

    traceotter_metadata_keys = [
        "traceotter_prompt",
    ]
