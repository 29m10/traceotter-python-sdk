"""If you use the OpenAI Python SDK, you can use the Traceotter drop-in replacement to get full logging by changing only the import.

```diff
- import openai
+ from traceotter.openai import openai
```

Traceotter automatically tracks:
...
See docs for more details: https://traceotter.com/docs/integrations/openai
"""
...
from traceotter._client.get_client import get_client
from traceotter._client.span import TraceotterGeneration
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["traceotter_public_key"] = traceotter_public_key
        self.args["traceotter_prompt"] = traceotter_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_traceotter_args(self) -> Any:
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to traceotter
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to traceotter
                ...
                processed_content.append(
                    {
                        "type": "input_audio",
                        "input_audio": {
                            "data": TraceotterMedia(base64_data_uri=base64_data_uri),
                            "format": format,
                        },
                    }
                )
...
def _extract_chat_response(kwargs: Any) -> Any:
    ...
    if kwargs.get("audio") is not None:
        ...
        if "data" in audio and audio["data"] is not None:
            ...
            audio["data"] = TraceotterMedia(base64_data_uri=base64_data_uri)
...
def _get_traceotter_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    default_name = (
    ...
    traceotter_public_key = kwargs.get("traceotter_public_key", None)
    if traceotter_public_key is not None and not isinstance(traceotter_public_key, str):
        raise TypeError("traceotter_public_key must be a string")
    ...
    traceotter_prompt = kwargs.get("traceotter_prompt", None)
    ...
    return {
        ...
        "model": model or None,
        "prompt": traceotter_prompt,
    }


def _create_traceotter_update(
    completion: Any,
...
def _get_traceotter_data_from_default_response(
...
def _wrap(
    ...
    traceotter_args = arg_extractor.get_traceotter_args()

    traceotter_data = _get_traceotter_data_from_kwargs(open_ai_resource, traceotter_args)
    traceotter_client = get_client(public_key=traceotter_args["traceotter_public_key"])

    observation_type = (
        "embedding" if open_ai_resource.type == "embedding" else "generation"
    )

    generation = traceotter_client.start_observation(
        as_type=observation_type,  # type: ignore
        name=traceotter_data["name"],
        input=traceotter_data.get("input", None),
        metadata=traceotter_data.get("metadata", None),
        model_parameters=traceotter_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, traceotter_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, traceotter_data.get("parent_observation_id", None)
            ),
        },
        model=traceotter_data.get("model", None),
        prompt=traceotter_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return TraceotterResponseGeneratorSync(
            ...
            model, completion, usage = _get_traceotter_data_from_default_response(
                open_ai_resource,
...
async def _wrap_async(
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return TraceotterResponseGeneratorAsync(
                resource=open_ai_resource,
                response=openai_response,
                generation=generation,
            )
...
class TraceotterResponseGeneratorSync:
    def __init__(
        self,
        *,
        resource: Any,
...
class TraceotterResponseGeneratorAsync:
    def __init__(
        self,
        *,
        resource: Any,
