"""Public API for commonly used Traceotter types.

This module provides a centralized location for importing commonly used types
from the Traceotter SDK, making them easily accessible without requiring
nested imports.

Example:
    ```python
    from traceotter.types import ObservationParams, ParsedMediaReference
    ```
"""
...
from traceotter.api import MediaContentType, UsageDetails
from traceotter.model import MapValue, ModelUsage, PromptClient
...
class ObservationParams(TraceMetadata, total=False):
    input: Optional[Any]
    output: Optional[Any]
