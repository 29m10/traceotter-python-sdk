"""This module contains the TraceotterMedia class, which is used to wrap media objects for upload to Traceotter."""
...
from traceotter.api import MediaContentType
from traceotter.types import ParsedMediaReference

T = TypeVar("T")


class TraceotterMedia:
    """A class for wrapping media objects for upload to Traceotter.

    This class handles the preparation and formatting of media content for Traceotter,
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a TraceotterMedia object.
        ...
        if base64_data_uri is not None:
            ...
            self._log.error(
                "base64_data_uri, or content_bytes and content_type, or file_path must be provided to TraceotterMedia"
            )
    ...
    def _reference_string(self) -> Optional[str]:
        ...
        return f"@@@traceotterMedia:type={self._content_type}|id={self._media_id}|source={self._source}@@@"
    ...
    def parse_reference_string(reference_string: str) -> ParsedMediaReference:
        ...
        Example reference string:
            "@@@traceotterMedia:type=image/jpeg|id=some-uuid|source=base64_data_uri@@@"
        ...
        Raises:
            ...
            ValueError: If the reference string does not start with "@@@traceotterMedia:type=".
        ...
        if not reference_string.startswith("@@@traceotterMedia:type="):
            raise ValueError(
                "Reference string does not start with '@@@traceotterMedia:type='"
            )

        if not reference_string.endswith("@@@"):
            raise ValueError("Reference string does not end with '@@@'")

        content = reference_string[len("@@@traceotterMedia:") :].rstrip("@@@")
    ...
    def resolve_media_references(
        *,
        ...
        Args:
            ...
            traceotter_client: Traceotter client instance used to fetch media content.
        ...
        def traverse(obj: Any, depth: int) -> Any:
            ...
            if isinstance(obj, str):
                ...
                for reference_string in reference_string_matches:
                    try:
                        parsed_media_reference = TraceotterMedia.parse_reference_string(
                            reference_string
                        )
                        media_data = traceotter_client.api.media.get(
