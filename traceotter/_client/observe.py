...
from traceotter._client.get_client import _set_current_public_key, get_client
...
from traceotter.types import TraceContext
...
class TraceotterDecorator:
    """Implementation of the @observe decorator for seamless Traceotter tracing integration.

    This class provides the core functionality for the @observe decorator, which enables
    automatic tracing of functions and methods in your application with Traceotter.
    It handles both synchronous and asynchronous functions, maintains proper trace context,
    and intelligently routes to the correct Traceotter client instance.
    ...
    - Thread-safe client resolution when multiple Traceotter projects are used
    """
    ...
    def observe(
        ...
        *,
        ...
        """Wrap a function to create and manage Traceotter tracing around its execution, supporting both synchronous and asynchronous functions.

        This decorator provides seamless integration of Traceotter observability into your codebase. It automatically creates
        ...
        Args:
            ...
            as_type (Optional[Literal]): Set the observation type. Supported values:
                    ...
                    Observation types are highlighted in the Traceotter UI for filtering and visualization.
                    The types "generation" and "embedding" create a span on which additional attributes such as model metrics
                    can be set.

        Returns:
            Callable: A wrapped version of the original function that automatically creates and manages Traceotter spans.
        ...
        Notes:
            ...
            - Special keyword arguments can be passed to control tracing:
              - traceotter_trace_id: Explicitly set the trace ID for this function call
              - traceotter_parent_observation_id: Explicitly set the parent span ID
              - traceotter_public_key: Use a specific Traceotter project (when multiple clients exist)
        ...
        function_io_capture_enabled = os.environ.get(
            TRACEOTTER_OBSERVE_DECORATOR_IO_CAPTURE_ENABLED, "True"
    ...
    def _async_observe(
        ...
        *,
        ...
        async def async_wrapper(*args: Tuple[Any], **kwargs: Dict[str, Any]) -> Any:
            trace_id = cast(str, kwargs.pop("traceotter_trace_id", None))
            parent_observation_id = cast(
                str, kwargs.pop("traceotter_parent_observation_id", None)
            )
            ...
            public_key = cast(str, kwargs.pop("traceotter_public_key", None))

            # Set public key in execution context for nested decorated functions
            with _set_current_public_key(public_key):
                traceotter_client = get_client(public_key=public_key)
                context_manager: Optional[
                    ...
                    traceotter_client.start_as_current_observation(
                        name=final_name,
                ...
                with context_manager as traceotter_span_or_generation:
                    ...
                    try:
                        ...
                        if capture_output is True:
                            ...
                            traceotter_span_or_generation.update(output=result)

                        return result
                    except Exception as e:
                        traceotter_span_or_generation.update(
                        ...
                        if not is_return_type_generator:
                            traceotter_span_or_generation.end()
    ...
    def _sync_observe(
        ...
        *,
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            trace_id = kwargs.pop("traceotter_trace_id", None)
            parent_observation_id = kwargs.pop("traceotter_parent_observation_id", None)
            trace_context: Optional[TraceContext] = (
                {
                    "trace_id": trace_id,
                    "parent_span_id": parent_observation_id,
            ...
            public_key = kwargs.pop("traceotter_public_key", None)
    ...
    def _wrap_sync_generator_result(
        self,
        traceotter_span_or_generation: Union[
            TraceotterSpan,
            TraceotterGeneration,
            TraceotterAgent,
            TraceotterTool,
...
_decorator = TraceotterDecorator()
