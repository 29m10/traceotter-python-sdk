"""Tracer implementation for Traceotter OpenTelemetry integration.

This module provides the TraceotterTracer class, a thread-safe singleton that manages OpenTelemetry
tracing infrastructure for Traceotter. It handles tracer initialization, span processors,
API clients, and coordinates background tasks for efficient data processing and media handling.

Key features:
- Thread-safe OpenTelemetry tracer with Traceotter-specific span processors and sampling
...
from traceotter._client.environment_variables import (
    TRACEOTTER_MEDIA_UPLOAD_ENABLED,
    TRACEOTTER_MEDIA_UPLOAD_THREAD_COUNT,
    TRACEOTTER_RELEASE,
    TRACEOTTER_TRACING_ENVIRONMENT,
)
from traceotter._client.span_processor import TraceotterSpanProcessor
from traceotter._task_manager.media_manager import MediaManager
from traceotter._task_manager.media_upload_consumer import MediaUploadConsumer
from traceotter._task_manager.score_ingestion_consumer import ScoreIngestionConsumer
from traceotter._utils.environment import get_common_release_envs
from traceotter._utils.prompt_cache import PromptCache
from traceotter._utils.request import TraceotterClient
from traceotter.api.client import AsyncFernTraceotter, FernTraceotter
from traceotter.logger import traceotter_logger
from traceotter.types import MaskFunction

from ..version import __version__ as traceotter_version


class TraceotterResourceManager:
    ...
    2. Managing the span processor for exporting spans to the Traceotter API
    3. Creating and managing Traceotter API clients (both synchronous and asynchronous)
    ...
    _instances: Dict[str, "TraceotterResourceManager"] = {}
    ...
    def _initialize_instance(
        ...
        *,
        ...
        if tracing_enabled:
            ...
            traceotter_processor = TraceotterSpanProcessor(
            ...
            tracer_provider.add_span_processor(traceotter_processor)
        ...
        self.api = FernTraceotter(
            base_url=host,
            username=self.public_key,
            password=secret_key,
            x_traceotter_sdk_name="python",
            x_traceotter_sdk_version=traceotter_version,
            x_traceotter_public_key=self.public_key,
            httpx_client=self.httpx_client,
            timeout=timeout,
        )
        self.async_api = AsyncFernTraceotter(
            base_url=host,
            username=self.public_key,
            password=secret_key,
            x_traceotter_sdk_name="python",
        ...
        score_ingestion_client = TraceotterClient(
            public_key=self.public_key,
            secret_key=secret_key,
            base_url=host,
        ...
        self._media_upload_enabled = os.environ.get(
            TRACEOTTER_MEDIA_UPLOAD_ENABLED, "True"
        ...
        media_upload_thread_count = media_upload_thread_count or max(
            int(os.getenv(TRACEOTTER_MEDIA_UPLOAD_THREAD_COUNT, 1)), 1
        )
        ...
        traceotter_logger.info(
            f"Startup: Traceotter tracer successfully initialized | "
    ...
    def flush(self) -> None:
        ...
        if not isinstance(tracer_provider, otel_trace_api.ProxyTracerProvider):
            ...
            traceotter_logger.debug("Successfully flushed OTEL tracer provider")

        self._score_ingestion_queue.join()
        traceotter_logger.debug("Successfully flushed score ingestion queue")

        self._media_upload_queue.join()
        traceotter_logger.debug("Successfully flushed media upload queue")
...
def _init_tracer_provider(
    *,
    ...
    environment = environment or os.environ.get(TRACEOTTER_TRACING_ENVIRONMENT)
    release = release or os.environ.get(TRACEOTTER_RELEASE) or get_common_release_envs()

    resource_attributes = {
        TraceotterOtelSpanAttributes.ENVIRONMENT: environment,
