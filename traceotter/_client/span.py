"""OTEL span wrapper for Traceotter.

This module defines custom span classes that extend OpenTelemetry spans with
Traceotter-specific functionality. These wrapper classes provide methods for
creating, updating, and scoring various types of spans used in AI application tracing.

Classes:
- TraceotterObservationWrapper: Abstract base class for all Traceotter spans
- TraceotterSpan: Implementation for general-purpose spans
- TraceotterGeneration: Specialized span implementation for LLM generations

All span classes provide methods for media processing, attribute management,
and scoring integration specific to Traceotter's observability platform.
"""
...
if TYPE_CHECKING:
    from traceotter._client.client import Traceotter

from traceotter._client.attributes import (
...
from traceotter.types import MapValue, ScoreDataType, SpanLevel
...
_OBSERVATION_CLASS_MAP: Dict[str, Type["TraceotterObservationWrapper"]] = {}


class TraceotterObservationWrapper:
    """Abstract base class for all Traceotter span types.

    This class provides common functionality for all Traceotter span types, including
    media processing, attribute management, and scoring. It wraps an OpenTelemetry
    span and extends it with Traceotter-specific features.

    Attributes:
        _otel_span: The underlying OpenTelemetry span
        _traceotter_client: Reference to the parent Traceotter client
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new Traceotter span wrapper.

        Args:
            otel_span: The OpenTelemetry span to wrap
            traceotter_client: Reference to the parent Traceotter client
        ...
        self._otel_span.set_attribute(
            TraceotterOtelSpanAttributes.OBSERVATION_TYPE, as_type
        )
        self._traceotter_client = traceotter_client
        self._observation_type = as_type

        self.trace_id = self._traceotter_client._get_otel_trace_id(otel_span)
        self.id = self._traceotter_client._get_otel_span_id(otel_span)

        self._environment = environment or self._traceotter_client._environment
        if self._environment is not None:
            self._otel_span.set_attribute(
                TraceotterOtelSpanAttributes.ENVIRONMENT, self._environment
            )
        ...
        if self._otel_span.is_recording():
            ...
            attributes.pop(TraceotterOtelSpanAttributes.OBSERVATION_TYPE, None)
    ...
    def end(self, *, end_time: Optional[int] = None) -> "TraceotterObservationWrapper":
    ...
    def score(
        ...
        *,
        ...
        Example:
            ...
            ```
        """
        self._traceotter_client.create_score(
            name=name,
    ...
    def _mask_attribute(self, *, data: Any) -> Any:
        ...
        """
        if not self._traceotter_client._mask:
            return data

        try:
            return self._traceotter_client._mask(data=data)
    ...
    def _process_media_in_attribute(
        ...
        *,
        ...
        """
        if self._traceotter_client._resources is not None:
            return (
                self._traceotter_client._resources._media_manager._find_and_process_media(
                    data=data,
                    field=field,
                    trace_id=self.trace_id,
                    observation_id=self.id,
    ...
    def _set_otel_span_status_if_error(
        ...
        Traceotter observation level is set to ERROR, ensuring consistency between
        Traceotter and OpenTelemetry error states.
    ...
    def start_observation(
        ...
        *,
        ...
        if as_type == "event":
            timestamp = time_ns()
            event_span = self._traceotter_client._otel_tracer.start_span(
            ...
            return cast(
                ...
                TraceotterEvent(
                    ...
                    traceotter_client=self._traceotter_client,
                    input=input,
                    output=output,
                    metadata=metadata,
        ...
        if not observation_class:
            traceotter_logger.warning(
                f"Unknown observation type: {as_type}, falling back to TraceotterSpan"
            )
            observation_class = TraceotterSpan

        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._traceotter_client._otel_tracer.start_span(name=name)
    ...
    def start_as_current_observation(  # type: ignore[misc]
        ...
        *,
        ...
        """
        return self._traceotter_client._create_span_with_parent_context(
            name=name,
            as_type=as_type,
...
class TraceotterSpan(TraceotterObservationWrapper):
    """Standard span implementation for general operations in Traceotter.

    This class represents a general-purpose span that can be used to trace
    any operation in your application. It extends the base TraceotterObservationWrapper
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new TraceotterSpan.
        ...
        super().__init__(
            otel_span=otel_span,
            as_type="span",
            traceotter_client=traceotter_client,
            input=input,
            output=output,
            metadata=metadata,
    ...
    def start_span(
        ...
        Returns:
            A new TraceotterSpan that must be ended with .end() when complete

        Example:
            ```python
            parent_span = traceotter.start_span(name="process-request")
            try:
                # Create a child span
    ...
    def start_as_current_span(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new TraceotterSpan

        Example:
            ```python
            with traceotter.start_as_current_span(name="process-request") as parent_span:
    ...
    def start_generation(
        ...
        *,
        ...
        Returns:
            A new TraceotterGeneration that must be ended with .end() when complete

        Example:
            ```python
            span = traceotter.start_span(name="process-query")
            try:
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new TraceotterGeneration
    ...
    def create_event(
        ...
        *,
        ...
        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._traceotter_client._otel_tracer.start_span(
                name=name, start_time=timestamp
            )
...
class TraceotterGeneration(TraceotterObservationWrapper):
    """Specialized span implementation for AI model generations in Traceotter.

    This class represents a generation span specifically designed for tracking
    AI/LLM operations. It extends the base TraceotterObservationWrapper with specialized
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new TraceotterGeneration span.
...
class TraceotterEvent(TraceotterObservationWrapper):
    """Specialized span implementation for Traceotter Events."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new TraceotterEvent span.
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        """Update is not allowed for TraceotterEvent because events cannot be updated.

        This method logs a warning and returns self without making changes.

        Returns:
            self: Returns the unchanged TraceotterEvent instance
        """
        traceotter_logger.warning(
            "Attempted to update TraceotterEvent observation. Events cannot be updated after creation."
        )
        return self


class TraceotterAgent(TraceotterObservationWrapper):
    """Agent observation for reasoning blocks that act on tools using LLM guidance."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterAgent span."""
        kwargs["as_type"] = "agent"
        super().__init__(**kwargs)


class TraceotterTool(TraceotterObservationWrapper):
    """Tool observation representing external tool calls, e.g., calling a weather API."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterTool span."""
        kwargs["as_type"] = "tool"
        super().__init__(**kwargs)


class TraceotterChain(TraceotterObservationWrapper):
    """Chain observation for connecting LLM application steps, e.g. passing context from retriever to LLM."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterChain span."""
        kwargs["as_type"] = "chain"
        super().__init__(**kwargs)


class TraceotterRetriever(TraceotterObservationWrapper):
    """Retriever observation for data retrieval steps, e.g. vector store or database queries."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterRetriever span."""
        kwargs["as_type"] = "retriever"
        super().__init__(**kwargs)


class TraceotterEmbedding(TraceotterObservationWrapper):
    """Embedding observation for LLM embedding calls, typically used before retrieval."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterEmbedding span."""
        kwargs["as_type"] = "embedding"
        super().__init__(**kwargs)


class TraceotterEvaluator(TraceotterObservationWrapper):
    """Evaluator observation for assessing relevance, correctness, or helpfulness of LLM outputs."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterEvaluator span."""
        kwargs["as_type"] = "evaluator"
        super().__init__(**kwargs)


class TraceotterGuardrail(TraceotterObservationWrapper):
    """Guardrail observation for protection e.g. against jailbreaks or offensive content."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new TraceotterGuardrail span."""
...
_OBSERVATION_CLASS_MAP.update(
    {
        ...
        "agent": TraceotterAgent,
        "tool": TraceotterTool,
        "chain": TraceotterChain,
        "retriever": TraceotterRetriever,
        "evaluator": TraceotterEvaluator,
        "embedding": TraceotterEmbedding,
        "guardrail": TraceotterGuardrail,
    }
)
