"""Traceotter OpenTelemetry integration module.

This module implements Traceotter's core observability functionality on top of the OpenTelemetry (OTel) standard.
"""
...
from traceotter._client.attributes import TraceotterOtelSpanAttributes
...
from traceotter._client.datasets import DatasetClient, DatasetItemClient
from traceotter._client.environment_variables import (
    TRACEOTTER_DEBUG,
    TRACEOTTER_HOST,
    TRACEOTTER_PUBLIC_KEY,
    TRACEOTTER_SAMPLE_RATE,
    TRACEOTTER_SECRET_KEY,
    TRACEOTTER_TIMEOUT,
    TRACEOTTER_TRACING_ENABLED,
    TRACEOTTER_TRACING_ENVIRONMENT,
)
from traceotter._client.resource_manager import TraceotterResourceManager
from traceotter._client.span import (
    TraceotterAgent,
    TraceotterChain,
    TraceotterEmbedding,
    TraceotterEvaluator,
...
from traceotter._client.utils import run_async_safely
from traceotter._utils import _get_timestamp
from traceotter._utils.parse_error import handle_fern_exception
from traceotter._utils.prompt_cache import PromptCache
from traceotter.api.resources.commons.errors.error import Error
from traceotter.api.resources.ingestion.types.score_body import ScoreBody
from traceotter.api.resources.prompts.types import (
    CreatePromptRequest_Chat,
    CreatePromptRequest_Text,
    Prompt_Chat,
    Prompt_Text,
)
from traceotter.experiment import (
    Evaluation,
    EvaluatorFunction,
    ExperimentData,
    ExperimentItem,
...
from traceotter.media import TraceotterMedia
from traceotter.model import (
    ChatMessageDict,
...
from traceotter.types import MaskFunction, ScoreDataType, SpanLevel, TraceContext


class Traceotter:
    """Main client for Traceotter tracing and platform features.

    This class provides an interface for creating and managing traces, spans,
    and generations in Traceotter as well as interacting with the Traceotter API.
    ...
    Attributes:
        api: Synchronous API client for Traceotter backend communication
        async_api: Asynchronous API client for Traceotter backend communication
        _otel_tracer: Internal TraceotterTracer instance managing OpenTelemetry components

    Parameters:
        public_key (Optional[str]): Your Traceotter public API key. Can also be set via TRACEOTTER_PUBLIC_KEY environment variable.
        secret_key (Optional[str]): Your Traceotter secret API key. Can also be set via TRACEOTTER_SECRET_KEY environment variable.
        host (Optional[str]): The Traceotter API host URL. Defaults to "https://cloud.traceotter.com". Can also be set via TRACEOTTER_HOST environment variable.
        timeout (Optional[int]): Timeout in seconds for API requests. Defaults to 5 seconds.
        httpx_client (Optional[httpx.Client]): Custom httpx client for making non-tracing HTTP requests. If not provided, a default client will be created.
        debug (bool): Enable debug logging. Defaults to False. Can also be set via TRACEOTTER_DEBUG environment variable.
        tracing_enabled (Optional[bool]): Enable or disable tracing. Defaults to True. Can also be set via TRACEOTTER_TRACING_ENABLED environment variable.
        flush_at (Optional[int]): Number of spans to batch before sending to the API. Defaults to 512. Can also be set via TRACEOTTER_FLUSH_AT environment variable.
        flush_interval (Optional[float]): Time in seconds between batch flushes. Defaults to 5 seconds. Can also be set via TRACEOTTER_FLUSH_INTERVAL environment variable.
        environment (Optional[str]): Environment name for tracing. Default is 'default'. Can also be set via TRACEOTTER_TRACING_ENVIRONMENT environment variable. Can be any lowercase alphanumeric string with hyphens and underscores that does not start with 'traceotter'.
        release (Optional[str]): Release version/hash of your application. Used for grouping analytics by release.
        media_upload_thread_count (Optional[int]): Number of background threads for handling media uploads. Defaults to 1. Can also be set via TRACEOTTER_MEDIA_UPLOAD_THREAD_COUNT environment variable.
        sample_rate (Optional[float]): Sampling rate for traces (0.0 to 1.0). Defaults to 1.0 (100% of traces are sampled). Can also be set via TRACEOTTER_SAMPLE_RATE environment variable.
        mask (Optional[MaskFunction]): Function to mask sensitive data in traces before sending to the API.
        blocked_instrumentation_scopes (Optional[List[str]]): List of instrumentation scope names to block from being exported to Traceotter. Spans from these scopes will be filtered out before being sent to the API. Useful for filtering out spans from specific libraries or frameworks. For exported spans, you can see the instrumentation scope name in the span metadata in Traceotter (`metadata.scope.name`)
        ...
        tracer_provider(Optional[TracerProvider]): OpenTelemetry TracerProvider to use for Traceotter. This can be useful to set to have disconnected tracing between Traceotter and other OpenTelemetry-span emitting libraries. Note: To track active spans, the context is still shared between TracerProviders. This may lead to broken trace trees.

    Example:
        ```python
        from traceotter.otel import Traceotter

        # Initialize the client (reads from env vars if not provided)
        traceotter = Traceotter(
            public_key="your-public-key",
            secret_key="your-secret-key",
            host="https://cloud.traceotter.com",  # Optional, default shown
        )

        # Create a trace span
        with traceotter.start_as_current_span(name="process-query") as span:
    ...
    _resources: Optional[TraceotterResourceManager] = None
    ...
    def __init__(
        ...
        *,
        ...
        self._host = host or cast(
            str, os.environ.get(TRACEOTTER_HOST, "https://cloud.traceotter.com")
        )
        self._environment = environment or cast(
            str, os.environ.get(TRACEOTTER_TRACING_ENVIRONMENT)
        )
        self._project_id: Optional[str] = None
        sample_rate = sample_rate or float(os.environ.get(TRACEOTTER_SAMPLE_RATE, 1.0))
        if not 0.0 <= sample_rate <= 1.0:
            raise ValueError(
                f"Sample rate must be between 0.0 and 1.0, got {sample_rate}"
            )

        timeout = timeout or int(os.environ.get(TRACEOTTER_TIMEOUT, 5))

        self._tracing_enabled = (
            tracing_enabled
            and os.environ.get(TRACEOTTER_TRACING_ENABLED, "true").lower() != "false"
        )
        if not self._tracing_enabled:
            traceotter_logger.info(
                "Configuration: Traceotter tracing is explicitly disabled. No data will be sent to the Traceotter API."
            )

        debug = (
            debug if debug else (os.getenv(TRACEOTTER_DEBUG, "false").lower() == "true")
        )
        if debug:
            logging.basicConfig(
                format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
            )
            traceotter_logger.setLevel(logging.DEBUG)

        public_key = public_key or os.environ.get(TRACEOTTER_PUBLIC_KEY)
        if public_key is None:
            traceotter_logger.warning(
                "Authentication error: Traceotter client initialized without public_key. Client will be disabled. "
                "Provide a public_key parameter or set TRACEOTTER_PUBLIC_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return

        secret_key = secret_key or os.environ.get(TRACEOTTER_SECRET_KEY)
        if secret_key is None:
            traceotter_logger.warning(
                "Authentication error: Traceotter client initialized without secret_key. Client will be disabled. "
                "Provide a secret_key parameter or set TRACEOTTER_SECRET_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return
        ...
        if os.environ.get("OTEL_SDK_DISABLED", "false").lower() == "true":
            traceotter_logger.warning(
                "OTEL_SDK_DISABLED is set. Traceotter tracing will be disabled and no traces will appear in the UI."
            )

        # Initialize api and tracer if requirements are met
        self._resources = TraceotterResourceManager(
    ...
    def start_span(
        ...
        *,
        ...
        Returns:
            A TraceotterSpan object that must be ended with .end() when the operation completes

        Example:
            ```python
            span = traceotter.start_span(name="process-data")
            try:
                # Do work
    ...
    def start_as_current_span(
        ...
        *,
    ...
    ) -> _AgnosticContextManager[TraceotterSpan]:
        ...
        Returns:
            A context manager that yields a TraceotterSpan
        ...
        if trace_context:
            ...
            if trace_id:
                ...
                with otel_trace_api.use_span(
                    ...
                    otel_span.set_attribute(TraceotterOtelSpanAttributes.AS_ROOT, True)
    ...
    def _create_observation_from_otel_span(
        ...
        *,
        ...
        if as_type in get_observation_types_list(ObservationTypeGenerationLike):
            ...
            # issue is that TraceotterEvent could be returned and that classes have diff. args
            return observation_class(  # type: ignore[return-value,call-arg]
                otel_span=otel_span,
                traceotter_client=self,
                environment=self._environment,
                input=input,
                output=output,
                metadata=metadata,
            ...
            # span._otel_span.set_attribute("traceotter.observation.type", as_type)
            # return span
    ...
    def start_generation(
        ...
        *,
        ...
        Args:
            ...
            prompt: Associated prompt template from Traceotter prompt management

        Returns:
            A TraceotterGeneration object that must be ended with .end() when complete

        Example:
            ```python
            generation = traceotter.start_generation(
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        prompt: Optional[PromptClient] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterGeneration]:
        ...
        Returns:
            A context manager that yields a TraceotterGeneration

        Example:
            ```python
            with traceotter.start_as_current_generation(
                name="answer-generation",
                model="gpt-4",
                input={"prompt": "Explain quantum computing"}
            ) as generation:
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterAgent]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterTool]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterChain]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterRetriever]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterEvaluator]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        prompt: Optional[PromptClient] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterEmbedding]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[TraceotterGuardrail]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        Example:
            ```python
            # Create a span
            with traceotter.start_as_current_observation(name="process-query", as_type="span") as span:
                # Do work
                result = process_data()
                span.update(output=result)
            ...
            with traceotter.start_as_current_observation(name="web-search", as_type="tool") as tool:
                # Do tool work
                results = search_web(query)
                tool.update(output=results)

            # Create a generation observation
            with traceotter.start_as_current_observation(
                name="answer-generation",
                as_type="generation",
                model="gpt-4"
            ) as generation:
    ...
    def _get_span_class(
        ...
        Type[TraceotterAgent],
        Type[TraceotterTool],
        Type[TraceotterChain],
        Type[TraceotterRetriever],
        Type[TraceotterEvaluator],
        Type[TraceotterEmbedding],
        Type[TraceotterGuardrail],
        Type[TraceotterGeneration],
        Type[TraceotterEvent],
        Type[TraceotterSpan],
    ]:
        ...
        with otel_trace_api.use_span(parent_span):
            with self._start_as_current_otel_span_with_processed_media(
                ...
                if remote_parent_span is not None:
                    traceotter_span._otel_span.set_attribute(
                        TraceotterOtelSpanAttributes.AS_ROOT, True
                    )

                yield traceotter_span
    ...
    def update_current_generation(
        ...
        *,
        ...
        Example:
            ```python
            with traceotter.start_as_current_generation(name="answer-query") as generation:
                # Initial setup and API call
                response = llm.generate(...)

                # Update with results that weren't available at creation time
                traceotter.update_current_generation(
                    output=response.text,
                    usage_details={
                        "prompt_tokens": response.usage.prompt_tokens,
                        "completion_tokens": response.usage.completion_tokens
        ...
        if current_otel_span is not None:
            generation = TraceotterGeneration(
                otel_span=current_otel_span, traceotter_client=self
            )
    ...
    def update_current_span(
        ...
        *,
        ...
        Example:
            ```python
            with traceotter.start_as_current_span(name="process-data") as span:
                # Initial processing
                result = process_first_part()

                # Update with intermediate results
                traceotter.update_current_span(metadata={"intermediate_result": result})

                # Continue processing
                final_result = process_second_part(result)

                # Final update
                traceotter.update_current_span(output=final_result)
            ```
        """
        if not self._tracing_enabled:
            traceotter_logger.debug(
    ...
    def update_current_trace(
        ...
        *,
        ...
        This method updates the Traceotter trace that the current span belongs to. It's useful for
        adding trace-level metadata like user ID, session ID, or tags that apply to
        the entire Traceotter trace rather than just a single observation.

        Args:
            name: Updated name for the Traceotter trace
            user_id: ID of the user who initiated the Traceotter trace
            session_id: Session identifier for grouping related Traceotter traces
            version: Version identifier for the application or service
            input: Input data for the overall Traceotter trace
            output: Output data from the overall Traceotter trace
            metadata: Additional metadata to associate with the Traceotter trace
            tags: List of tags to categorize the Traceotter trace
            public: Whether the Traceotter trace should be publicly accessible

        Example:
            ```python
            with traceotter.start_as_current_span(name="handle-request") as span:
                # Get user information
                user = authenticate_user(request)

                # Update trace with user context
                traceotter.update_current_trace(
                    user_id=user.id,
        ...
        if current_otel_span is not None:
            existing_observation_type = current_otel_span.attributes.get(  # type: ignore[attr-defined]
                TraceotterOtelSpanAttributes.OBSERVATION_TYPE, "span"
            )
    ...
    def create_event(
        ...
        *,
        ...
        """Create a new Traceotter observation of type 'EVENT'.

        The created Traceotter Event observation will be the child of the current span in the context.
        ...
        Returns:
            The Traceotter Event object

        Example:
            ```python
            event = traceotter.create_event(name="process-event")
            ```
        """
        timestamp = time_ns()
    ...
    def _create_remote_parent_span(
        ...
        if not self._is_valid_trace_id(trace_id):
            traceotter_logger.warning(
                f"Passed trace ID '{trace_id}' is not a valid 32 lowercase hex char Traceotter trace id. Ignoring trace ID."
            )

        if parent_span_id and not self._is_valid_span_id(parent_span_id):
            traceotter_logger.warning(
                f"Passed span ID '{parent_span_id}' is not a valid 16 lowercase hex char Traceotter span id. Ignoring parent span ID."
            )
    ...
    def _create_observation_id(self, *, seed: Optional[str] = None) -> str:
        """Create a unique observation ID for use with Traceotter.

        This method generates a unique observation ID (span ID in OpenTelemetry terms)
        for use with various Traceotter APIs. It can either generate a random ID or
        ...
        correlate an external ID with a Traceotter observation ID, use the external ID as
        ...
        Example:
            ...
            obs_id = traceotter.create_observation_id()

            # Generate a deterministic ID based on a seed
            user_obs_id = traceotter.create_observation_id(seed="user-123-feedback")

            # Correlate an external item ID with a Traceotter observation ID
            item_id = "item-789012"
            correlated_obs_id = traceotter.create_observation_id(seed=item_id)

            # Use the ID with Traceotter APIs
            traceotter.create_score(
                name="relevance",
                value=0.95,
                trace_id=trace_id,
    ...
    def create_trace_id(*, seed: Optional[str] = None) -> str:
        """Create a unique trace ID for use with Traceotter.

        This method generates a unique trace ID for use with various Traceotter APIs.
        ...
        This method ensures the generated ID meets this requirement. If you need to
        correlate an external ID with a Traceotter trace ID, use the external ID as the
        seed to get a valid, deterministic Traceotter trace ID.
        ...
        Returns:
            A 32-character lowercase hexadecimal string representing the Traceotter trace ID.

        Example:
            ```python
            # Generate a random trace ID
            trace_id = traceotter.create_trace_id()

            # Generate a deterministic ID based on a seed
            session_trace_id = traceotter.create_trace_id(seed="session-456")

            # Correlate an external ID with a Traceotter trace ID
            external_id = "external-system-123456"
            correlated_trace_id = traceotter.create_trace_id(seed=external_id)

            # Use the ID with trace context
            with traceotter.start_as_current_span(
        ...
        if not seed:
            ...
            return Traceotter._format_otel_trace_id(trace_id_int)
    ...
    def create_score(
        ...
        *,
        ...
        This method creates a score for evaluating a Traceotter trace or observation. Scores can be
        ...
        Args:
            ...
            session_id: ID of the Traceotter session to associate the score with
            dataset_run_id: ID of the Traceotter dataset run to associate the score with
            trace_id: ID of the Traceotter trace to associate the score with
            ...
            config_id: Optional ID of a score config defined in Traceotter
    ...
    def score_current_trace(
        ...
        *,
        ...
        Example:
            ```python
            with traceotter.start_as_current_span(name="process-user-request") as span:
                # Process request
                result = process_complete_request()
                span.update(output=result)

                # Score the overall trace
                traceotter.score_current_trace(
    ...
    def flush(self) -> None:
        """Force flush all pending spans and events to the Traceotter API.

        This method manually flushes any pending spans, scores, and other events to the
        Traceotter API. It's useful in scenarios where you want to ensure all data is sent
        before proceeding, without waiting for the automatic flush interval.

        Example:
            ```python
            # Record some spans and scores
            with traceotter.start_as_current_span(name="operation") as span:
                # Do work...
                pass

            # Ensure all data is sent to Traceotter before proceeding
            traceotter.flush()
    ...
    def shutdown(self) -> None:
        """Shut down the Traceotter client and flush all pending data.

        This method cleanly shuts down the Traceotter client, ensuring all pending data
        ...
        Example:
            ```python
            # Initialize Traceotter
            traceotter = Traceotter(public_key="...", secret_key="...")

            # Use Traceotter throughout your application
            # ...
    ...
    def get_current_trace_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with traceotter.start_as_current_span(name="process-request") as span:
                # Get the current trace ID for reference
                trace_id = traceotter.get_current_trace_id()
    ...
    def get_current_observation_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with traceotter.start_as_current_span(name="process-user-query") as span:
                # Get the current observation ID
                observation_id = traceotter.get_current_observation_id()
    ...
    def get_trace_url(self, *, trace_id: Optional[str] = None) -> Optional[str]:
        """Get the URL to view a trace in the Traceotter UI.

        This method generates a URL that links directly to a trace in the Traceotter UI.
        ...
        Returns:
            A URL string pointing to the trace in the Traceotter UI,
        ...
        Example:
            ...
            with traceotter.start_as_current_span(name="process-request") as span:
                trace_url = traceotter.get_trace_url()
                log.info(f"Processing trace: {trace_url}")

            # Get URL for a specific trace
            specific_trace_url = traceotter.get_trace_url(trace_id="1234567890abcdef1234567890abcdef")
    ...
    def get_dataset(
        ...
        """
        try:
            traceotter_logger.debug(f"Getting datasets {name}")
            ...
            items = [DatasetItemClient(i, traceotter=self) for i in dataset_items]
    ...
    def run_experiment(
        ...
        *,
        ...
        automatically traces all executions with Traceotter for observability, runs
        ...
        - Integration with Traceotter datasets for experiment tracking
        ...
        Args:
            name: Human-readable name for the experiment. Used for identification
                in the Traceotter UI.
            run_name: Optional exact name for the experiment run. If provided, this will be
                used as the exact dataset run name if the `data` contains Traceotter dataset items.
            ...
            data: Array of data items to process. Can be either:
                ...
                - List of Traceotter DatasetItem objects from dataset.items
            ...
            metadata: Optional metadata dictionary to attach to all experiment traces.
                ...
                If `data` are Traceotter dataset items, the metadata will be attached to the dataset run, too.

        Returns:
            ExperimentResult containing:
            - run_name: The experiment run name. This is equal to the dataset run name if experiment was on Traceotter dataset.
            - item_results: List of results for each processed item with outputs and evaluations
            - run_evaluations: List of aggregate evaluation results for the entire run
            - dataset_run_id: ID of the dataset run (if using Traceotter datasets)
            - dataset_run_url: Direct URL to view results in Traceotter UI (if applicable)
        ...
        Examples:
            ...
            result = traceotter.run_experiment(
            ...
            Using with Traceotter datasets:
            ```python
            # Get dataset from Traceotter
            dataset = traceotter.get_dataset("my-eval-dataset")
            ...
            # Results automatically linked to dataset in Traceotter UI
        ...
        Note:
            ...
            - All executions are automatically traced and visible in Traceotter UI
            - When using Traceotter datasets, results are automatically linked for easy comparison
        ...
        *,
        ...
        for i, result in enumerate(item_results):
            if isinstance(result, Exception):
                traceotter_logger.error(f"Item {i} failed: {result}")
        ...
        for run_evaluator in run_evaluators:
            try:
                ...
                traceotter_logger.error(f"Run evaluator failed: {e}")
        ...
        for evaluation in run_evaluations:
            try:
                ...
                traceotter_logger.error(f"Failed to store run evaluation: {e}")
            ...
            try:
                ...
                if hasattr(item, "id") and hasattr(item, "dataset_id"):
                    try:
                        ...
                        traceotter_logger.error(f"Failed to create dataset run item: {e}")
                ...
                for evaluator in evaluators:
                    try:
                        ...
                        traceotter_logger.error(f"Evaluator failed: {e}")
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset with the given name on Traceotter.
        ...
        Returns:
            Dataset: The created dataset as returned by the Traceotter API.
        """
        try:
            body = CreateDatasetRequest(
                name=name, description=description, metadata=metadata
            )
            traceotter_logger.debug(f"Creating datasets {body}")
    ...
    def create_dataset_item(
        ...
        *,
        ...
        Returns:
            DatasetItem: The created dataset item as returned by the Traceotter API.

        Example:
            ```python
            from traceotter import Traceotter

            traceotter = Traceotter()

            # Uploading items to the Traceotter dataset named "capital_cities"
            traceotter.create_dataset_item(
        ...
        try:
            ...
            traceotter_logger.debug(f"Creating dataset item {body}")
    ...
    def resolve_media_references(
        ...
        *,
        ...
        in the format "@@@traceotterMedia:...@@@". When found, it (synchronously) fetches the actual media content using
        the provided Traceotter client and replaces the reference string with a base64 data URI.
        ...
        Example:
            obj = {
                "image": "@@@traceotterMedia:type=image/jpeg|id=123|source=bytes@@@",
                "nested": {
                    "pdf": "@@@traceotterMedia:type=application/pdf|id=456|source=bytes@@@"
                }
            }

            result = await TraceotterMedia.resolve_media_references(obj, traceotter_client)
            ...
            # }
        """
        return TraceotterMedia.resolve_media_references(
            traceotter_client=self,
            obj=obj,
    ...
    def get_prompt(
        ...
        *,
        ...
        Keyword Args:
            ...
            fallback: Union[Optional[List[ChatMessageDict]], Optional[str]]: The prompt string to return if fetching the prompt fails. Important on the first call where no cached prompt is available. Follows Traceotter prompt formatting with double curly braces for variables. Defaults to None.
        ...
        traceotter_logger.debug(f"Getting prompt '{cache_key}'")
        ...
        if cached_prompt.is_expired():
            traceotter_logger.debug(f"Stale prompt '{cache_key}' found in cache.")
            try:
                # refresh prompt in background thread, refresh_prompt deduplicates tasks
                traceotter_logger.debug(f"Refreshing prompt '{cache_key}' in background.")
    ...
    def _fetch_prompt_and_update_cache(
        ...
        *,
        ...
        cache_key = PromptCache.generate_cache_key(name, version=version, label=label)
        traceotter_logger.debug(f"Fetching prompt '{cache_key}' from server...")
    ...
    def create_prompt(
        ...
        *,
        ...
        """Create a new prompt in Traceotter.
        ...
        """
        try:
            traceotter_logger.debug(f"Creating prompt {name=}, {labels=}")
    ...
    def update_prompt(
        ...
        *,
        ...
        """Update an existing prompt version in Traceotter. The Traceotter SDK prompt cache is invalidated for all prompts witht he specified name.

        Args:
            name (str): The name of the prompt to update.
            version (int): The version number of the prompt to update.
            new_labels (List[str], optional): New labels to assign to the prompt version. Labels are unique across versions. The "latest" label is reserved and managed by Traceotter. Defaults to [].

        Returns:
            Prompt: The updated prompt from the Traceotter API.
