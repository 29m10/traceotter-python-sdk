...
# Context variable to track the current traceotter_public_key in execution context
_current_public_key: ContextVar[Optional[str]] = ContextVar(
    "traceotter_public_key", default=None
)
...
def _create_client_from_instance(
    instance: "TraceotterResourceManager", public_key: Optional[str] = None
) -> Traceotter:
    """Create a Traceotter client from a resource manager instance with all settings preserved."""
    return Traceotter(
...
def get_client(*, public_key: Optional[str] = None) -> Traceotter:
    """Get or create a Traceotter client instance.

    Returns an existing Traceotter client or creates a new one if none exists. In multi-project setups,
    providing a public_key is required. Multi-project support is experimental - see Traceotter docs.
    ...
    Returns:
        Traceotter: Client instance in one of three states:
    ...
    Example:
        ...
        ```
    """
    with TraceotterResourceManager._lock:
        active_instances = TraceotterResourceManager._instances
        ...
        if not public_key:
            ...
            if len(active_instances) == 1:
                ...
                traceotter_logger.warning(
                    "No 'traceotter_public_key' passed to decorated function, but multiple traceotter clients are instantiated in current process. Skipping tracing for this function to avoid cross-project leakage."
                )
                return Traceotter(
            ...
            target_instance: Optional[TraceotterResourceManager] = active_instances.get(
                public_key, None
            )
            ...
            if target_instance is None:
                ...
                traceotter_logger.warning(
                    f"No Traceotter client with public key {public_key} has been initialized. Skipping tracing for decorated function."
                )
                return Traceotter(
                    tracing_enabled=False, public_key="fake", secret_key="fake"
                )
